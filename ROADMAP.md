...
| **Honor `prefers-reduced-motion`** in CSS and surface a `prefersReducedMotion` flag. The shipped CSS has a hover transition and zero reduced-motion handling, and the API exposes no motion signal. Cheap, fully additive insurance before any step-transition animation lands. | none | S | **P2** |
...
| Recommendation | Breaking | Effort | Priority |
| --- | --- | --- | --- |
| **Replace cloneElement injection with a context `useReportValidity` hook.** `React.cloneElement` injects `signalParent` onto the immediate element only, so wrapping a step in an `ErrorBoundary` / `memo` / layout silently breaks validity - which is exactly why `signalParent` is typed optional and every real step writes `signalParent?.({ isValid })`. A hook that reads the step's index from a per-step context and dispatches `SET_STEP_VALIDITY` composes through any wrapper depth, makes the reporter a non-optional hook return, and deletes `childrenWithProps` / `cloneElement`. The reducer machinery already exists; the net-new work is the per-step index context. Merge with the structured payload: `useReportValidity(result: StepValidity)`. | major | M | **P0** |
